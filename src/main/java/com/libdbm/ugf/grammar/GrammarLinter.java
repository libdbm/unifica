...
public final class GrammarLinter {
    ...
    private boolean containsStateAnnotation(final RuleElement elem) {
        return switch (elem) {
            ...
            case RuleElement.Alternation alt -> alt.options().stream().anyMatch(this::containsStateAnnotation);
    ...
    private void collectUndefinedNonterminals(
        ...
        switch (elem) {
            ...
            case RuleElement.Repetition rep -> collectUndefinedNonterminals(rep.element(), defined, context, issues);
            case RuleElement.Alternation alt -> {
                for (final var opt : alt.options()) {
